...
const ItemCard = (props) => {
    ...
    if (product?.name) {
        ...
        props.data.optionlist.map((r, id) => {
            require("../images/" + r.code + ".png");
        })

        const defaultOption = props.data.option;
        //console.log(props.data);


        const defaultprice = props.data.p_price;
        ...
        if (isColor(props.data.option)) {
            color = true;
        }

        let image = require("../images/" + option.code + ".png");
        //const optionList = props.data.map(r => {return r.p_shortdescription_eng});
            ...
            <Card sx={{width: "250px", boxShadow: 7, borderRadius: "20px", margin: "10px"}}>
                ...
                <Box sx={{backgroundColor: cardstyle.palette.buttons, height: "150px"}}>
                    <CardHeader sx={{paddingBottom: "0px"}}
                        ...
                        subheader={<Typography variant="h6">{props.data.price}</Typography>} />
